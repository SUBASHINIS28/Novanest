        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {startups.map((startup) => (
            <div key={startup._id} className="bg-white shadow-lg rounded-lg p-4 transition-transform transform hover:scale-105">
              ...
              <p className="text-gray-600 mb-2"><strong>Funding Goal:</strong> ${typeof startup.fundingGoal === 'string' ? startup.fundingGoal : startup.fundingGoal.toLocaleString()}</p>
              <p className="text-gray-600 mb-3">{startup.description.length > 100 ? `${startup.description.substring(0, 100)}...` : startup.description}</p>
